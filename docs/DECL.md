...
3. Wrapper enforces exact cardinality: `out_count == new_states.length` (equivalently, after desugaring, all per-field arrays must have equal length and that length must equal `out_count`).
