...
All of the attached test code in this chapter comes from `silverscript-lang/tests/kcc20_tests.rs`. If you want to inspect the source directly in the repository, that is the file to open.
...
1. an uninitialized `KCC20Minter` is created with a placeholder KCC20 covenant ID
2. `init` binds it to a newly created KCC20 covenant instance
3. each mint spends the KCC20 minter branch and the KCC20Minter together
4. every successful mint recreates a zero-amount KCC20 minter branch and also creates a separate recipient KCC20 output with the newly minted amount
5. the first recipient output is then spent like an ordinary KCC20 branch to a different pubkey owner
