...
fn load_contract_source(path: &Path) -> &'static str {
    ...
    let leaked: &'static str = Box::leak(expanded.into_boxed_str());
...
fn preprocess_contract_source(path: &Path, source: &str) -> String {
    ...
    let consts_path = path.parent().unwrap_or_else(|| Path::new(".")).join(&consts_file);
    let consts_source =
...
fn size_snapshots() -> Vec<SizeSnapshot> {
    vec![
        ...
        SizeSnapshot {
            ...
            expected_script_len: 2922,
            expected_instruction_count: 2146,
            expected_charged_op_count: 1496,
        },
        SizeSnapshot {
            name: "chess_mux.sil",
